...
public class LemonLime extends SubsystemBase {
	...
	public double getVisualJoyStick(){
		/**
		 * Custom calculation utilizing 'getAngularOffset' using custom coordinates.
		 */
		double n = -clamp(driveStickPID.calculate(getAngularOffset().getDegrees()),-1,1);

		/**
		 * Similar calculation utilizing 'getDirectionToHub' from WCP supplied code.
		 */
		// double n = clamp(
		// 	driveStickPID.calculate(swerve.getHeading().getDegrees(),
